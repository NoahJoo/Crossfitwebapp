...
def member_start_workout_alone(request):
    ...
    # 유저 몸무게 가져오기
    user_bodyweight_workout_list = Workout.objects.filter(member_id=user.member_id, workout='bodyweight').order_by(
        'date')
    user_bodyweight_list = [int(wo.reps) for wo in user_bodyweight_workout_list]
    user_alone_bodyweight_workout_list = Workout.objects.filter(member_id=user.member_id, workout='bodyweight',
                                                                class_pk='00').order_by('date')
    ...
    if len(user_alone_bodyweight_list) == 0:
        if len(user_bodyweight_list) == 0:
            ...
            class_pk_workout_list = Workout.objects.filter(member_id='0000', class_pk=bmi_class_pk).order_by('workout',
                                                                                                             'set')
        ...
        if prev_bmi_class_pk != curr_bmi_class_pk:
            class_pk_workout_list = Workout.objects.filter(member_id='0000', class_pk=curr_bmi_class_pk).order_by(
                'workout', 'set')
            print(len(class_pk_workout_list))
            for class_pk_workout in class_pk_workout_list:
                newly_created_workout = Workout.objects.create(
            ...
            member_workout_list = Workout.objects.filter(member_id=user.member_id, alone_or_group=0).order_by('date',
                                                                                                              'workout',
                                                                                                              'set')
            ...
            member_last_workout_wo_bodyweight_list = [wo for wo in member_workout_wo_bodyweight_list if
                                                      wo.date == member_last_workout_date]
    ...
    return render(request, 'member-start-workout-alone-recommend-page.html',
                  {'recommended_workout': recommended_workout,
                   'recommended_workout_dictionary': recommended_workout_dictionary, })
...
def member_record(request):
    ...
    user_workouts = [a for a in all_workouts if a.member_id == user.member_id]
    last_date = user_workouts[-1].date  # type: datetime.date
    member_last_date_workouts_wo_bodyweight = [a for a in user_workouts if
                                               a.workout != 'bodyweight' and a.date == last_date]
